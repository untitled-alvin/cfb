...
abstract class CFBEventSink<Event extends Object?> implements ErrorSink {
...
/// {@endtemplate}
abstract class CFB<Event, State> extends CFBBase<State>
    implements CFBEventSink<Event> {
  /// {@macro cfb}
  CFB(State initialState) : super(initialState);

  /// The current [CFBObserver] instance.
  static CFBObserver observer = const _DefaultCFBObserver();
  ...
  /// it will take precedence over the global transformer.
  ///
  /// See also:
  ///
  ...
  final _eventTransformer = CFB.transformer;

  /// Notifies the [CFB] of a new [event] which triggers
  ...
  ///
  /// * A [StateError] will be thrown if the cfb is closed and the
  ...
  /// Called whenever an [event] is [add]ed to the [CFB].
  /// A great spot to add logging/analytics at the individual [CFB] level.
  ///
  ...
  /// See also:
  ///
  /// * [CFBObserver.onEvent] for observing events globally.
  ///
  @protected
  @mustCallSuper
  void onEvent(Event event) {
    // ignore: invalid_use_of_protected_member
    _cfbObserver.onEvent(this, event);
  }
  ...
  ///
  /// ```dart
  /// class MyCFB extends CFB<MyEvent, MyState> {
  ///   MyCFB() : super(MyInitialState()) {
  ...
  /// ```
  ///
  /// Updates the state of the cfb to the provided [state].
  /// A cfb's state should only be updated by `emitting` a new `state`
  ...
  /// * [package:cfb_concurrency](https://pub.dev/packages/cfb_concurrency) for an
  /// opinionated set of event transformers.
  ///
  ...
  /// See also:
  ///
  /// * [CFBObserver.onTransition] for observing transitions globally.
  ///
  @protected
  @mustCallSuper
  void onTransition(Transition<Event, State> transition) {
    // ignore: invalid_use_of_protected_member
    _cfbObserver.onTransition(this, transition);
  }

  /// Closes the `event` and `state` `Streams`.
  /// This method should be called when a [CFB] is no longer needed.
  /// Once [close] is called, `events` that are [add]ed will not be
  /// processed.
  /// In addition, if [close] is called while `events` are still being
  /// processed, the [CFB] will finish processing the pending `events`.
  @mustCallSuper
  @override
...
class _DefaultCFBObserver extends CFBObserver {
  const _DefaultCFBObserver();
}
